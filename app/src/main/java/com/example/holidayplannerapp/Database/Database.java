package com.example.holidayplannerapp.Database;
...
public class Database extends SQLiteOpenHelper {
    ...
    public static final String DB_NAME = "holiday_planner_db";
    ...
    // Creating an update method using email address
    public boolean updateUserDataByEmail(String email, String firstName, String lastName, String newPassword) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();

        // Update the values you want to change
        values.put(AUTH_COL_1, firstName);
        values.put(AUTH_COL_2, lastName);
        values.put(AUTH_COL_4, newPassword);

        // Define the WHERE clause to identify the row to update (in this case, by email)
        String selection = AUTH_COL_3 + " = ?";
        String[] selectionArgs = { email };

        // Perform the update
        int updatedRows = db.update(AUTH_TABLE, values, selection, selectionArgs);
    ...
    public ArrayList<ProductDataModel> productfetchdata(int catid) {
        ...
        Cursor getdata = db.rawQuery("SELECT * FROM " + PRODUCT_TABLE + " WHERE " + PRODUCT_COL_10 + " = ? ORDER BY " + PRODUCT_ID +" DESC ", new String[]{String.valueOf(catid)});
